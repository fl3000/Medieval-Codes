<h1>Medieval Code Imitator</h1>
(Klosterfenster-Generator 1.0)

Some of the worlds oldest monastrys do show strange and beautiful patterns in their windows. Is there a secret code hidden?

This script tries to replicate the look of those medieval windows, however it doesn't really put a code inside it ;-)

<img width="857" height="832" alt="Medieval_Codes" src="https://github.com/user-attachments/assets/1a4637a3-1d1b-4dc9-a20f-582225b9bdb6" />

The code is a script-file for "Context-Free-Art" - Viewer ([www.contextfreeart.org](https://www.contextfreeart.org/))
